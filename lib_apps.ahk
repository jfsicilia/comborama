...
#include %A_ScriptDir%\lib_desktop.ahk
  ...
  LibAppsAutoExec:
    ...
    global ONE_COMMANDER_FOCUS_COMBO := "#!e"
  return
  ...
  /*
    ...
    appCombo -- (optional) If appCombo is present, it will be issued to focus 
                ...
                focus oneCommander).
    useKeyPirinha -- (optional) If true, it will use appRun paramenter in
                    keypirinha to launch app.
  ...
  */
  FocusOrLaunchApp(appExe := "", appTitle := "", appRun := "", hidden := true, appRunParam := "", forceLaunch := false, appCombo := "", useKeyPirinha := false) {
    ...
    if (appRun != "") {
      if (useKeyPirinha) {
        sendInput(">!{space}")        ; Open keypirinha.
        sendInput(appRun . "{Enter}") ; Open app through keypirinha.
      }
      else {
  ...
  */
  ...
  ; TODO
    return FocusOrLaunchApp(, "Blender", "Blender", hidden,,,,true)
  }
  ...
  */
  FocusOrLaunchFusion360(hidden := true) {
    return FocusOrLaunchApp("fusion360.exe",, "fusion360", hidden,,,,true)
  }
  ...
  */
  ...
  /*
    Focus or launch UPdf
    hidden -- If true forces to look for the app even if it's hidden or in 
              another desktop.
  */
  FocusOrLaunchUPdf(hidden := true) {
    return FocusOrLaunchApp("UPDF.exe",, "C:\Program Files (x86)\UPDF\UPDF.exe", hidden)
  }
  ...
  */
  ...
  ; TODO
    return FocusOrLaunchApp("cura.exe",, "cura", hidden,,,,true)
  }
  ...
  */
  FocusOrLaunchWhatsApp(hidden := true) {
    ...
    return FocusOrLaunchApp(, "WhatsApp", "WhatsApp", hidden,,,,true)
  }
