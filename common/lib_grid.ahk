...
LibGridAutoExec:
  ; Global variables defined in lib_desktop.ahk and lib_monitor.ahk
  global N_DESKTOPS, N_MONITORS
  ...
  ; top of the stack is the window that will be activated when that zone 
  ; is selected. See below _wndStacks initialization for object description.
  global _wndStacks:= []
  ...
  loop, % N_DESKTOPS {
    ...
    loop, % N_MONITORS {
      ...
      ; of a zone. Each element is an object with x1, y1, x2, y2 properties.
...
ResizeActiveWindowZoneBorder(start, end, pStart, pEnd, thd, dir
  ...
  ; If can't resize to the left, exit.
...
SwapWindowToZone(winTitle, desktop:=-1, monitor:=-1, zone:=1) {
  ...
  if (last > 0) {
    ...
    ;MoveWindowToZone("ahk_id " . destHwnd,,,origZone)
  ...
  ;MoveWindowToZone("ahk_id " . origHwnd,,,destZone)
}
...
;
; Return number of zone where row/col belongs (1..zones.length()). Returns
; default if row/col doesn't belong to any zone.
...
;
; Return number of zone where x/y belongs (1..zones.length()). Returns
; default if row/col doesn't belong to any zone boundary.
...
__UpdateActiveWindowLocationCallback(active, prev) {
  ...
  ;LOG.log(winclass . " blacklisted? " . _resizeBlacklist[winclass])
